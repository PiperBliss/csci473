 ...
 * TIMING total_s=... read_s=... compute_s=... write_s=... gflops_total=... gflops_compute=... m=... n=... p=...
 ...
 * - gflops_...: GigaFLOPS/sec (total ops / time)
 */
...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        /* GFLOPS calculation: m rows, each a dot product of length n.
         * Dot product = n mults + n adds = 2*n ops. Total ops = m * 2 * n.
         * Use long long to avoid overflow for large matrices. */
        long long total_ops = (long long)m * 2 * (long long)n;
        double gflops_total   = (total_s > 1e-9) ? (double)total_ops / total_s / 1e9 : 0.0;
        double gflops_compute = (compute_s > 1e-9) ? (double)total_ops / compute_s / 1e9 : 0.0;

        /* Machine-readable one-liner */
        printf("TIMING total_s=%.9f read_s=%.9f compute_s=%.9f write_s=%.9f gflops_total=%.6f gflops_compute=%.6f m=%d n=%d p=%d\n",
               total_s, read_s, compute_s, write_s, gflops_total, gflops_compute, m, n, world);
