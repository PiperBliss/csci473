...
def run_stencil(rows, cols, iters, threads, make_exe, pth_exe):
    ...
    try:
        subprocess.run([make_exe, str(rows), str(cols), initial_file],
...
def create_plots(results, problem_sizes, all_threads, iterations, results_dir, label):
    ...
    for size in problem_sizes:
        ...
        organized_data[size] = {'threads': [], 'times': [], 'speedups': [], 'efficiency': []}
        for r in results:
    ...
    plt.title(f'Execution Time vs. Threads (I={I_label})')
    ...
    plt.xlabel('Threads (P)')
    plt.ylabel('Speedup')
    plt.title(f'Speedup vs. Threads (I={I_label})')
    plt.legend(title='Problem Size')
    plt.grid(True, linestyle=':')
    plt.xticks(all_threads)
    ...
    plt.title(f'Efficiency vs. Threads (I={I_label})')
    plt.legend(title='Problem Size')
    plt.grid(True, linestyle=':')
    plt.xticks(all_threads)
    ...
    for eff_idx, eff in enumerate(efficiencies):
        plot_threads, plot_n_values = [], []
        last_n_found = None

        for p in all_threads:
            min_n_for_p = None
            for n in sorted_problem_sizes:
                rec = next((r for r in results if
                            r['threads'] == p and
                            r['iterations'] == I_label and
                            r['size'] == f"{n}x{n}"), None)
                if rec and rec['efficiency'] >= eff:
                    min_n_for_p = n
                    break
            if min_n_for_p is not None:
                if last_n_found is None:
                    plot_threads.append(p)
                    plot_n_values.append(min_n_for_p)
                elif min_n_for_p != last_n_found:
                    plot_threads.append(p - 1)
